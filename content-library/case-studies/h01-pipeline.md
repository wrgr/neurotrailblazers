...
description: >
  A stage-by-stage walkthrough of how the H01 dataset was made: a fragment of human
  temporal cortex removed in epilepsy surgery, then staining, sectioning, 61-beam
  electron microscopy, alignment, segmentation and synapse detection, then the
  findings and their caveats. Figures are rendered from the public H01 volume.
topics:
  - human connectomics
...
H01 is a roughly cubic-millimeter fragment of human temporal cortex, imaged by
serial-section electron microscopy and reconstructed computationally. It is a
collaboration between the Lichtman laboratory at Harvard and the Connectomics at
Google team, published in *Science* in May 2024.

Every electron-microscopy figure here was rendered from the public H01 volume, the
same data you can open yourself. Nothing is an artist's impression.
...
|---|---|---|
| Physical sections cut | **5,019** (mean thickness 33.9 nm) | "5,293": that is the released volume's *z-layer count* (the `4nm_raw` info file; the 2021 preprint gives 5,292). About 5.8% of cuts came out double-thickness, and those are duplicated in the stack to keep the z-geometry honest. |
| Data volume | **1.8 PB** raw acquisition; **1.4 PB** aligned volume | The two are often conflated. Say which you mean. |
...
and the release page's numbers when you describe the downloadable data.** Never
average them.
...
Before the pipeline makes sense, you need a feel for the range it spans. These views are
all the same tissue, each roughly 4× closer than the last.

{% include figure.html
   src="/assets/images/content-library/case-studies/h01/01-whole-sample.jpg"
   alt="A single thin section of the H01 sample, a wedge-shaped fragment of human cortex about 4 millimeters across."
...
{% include figure.html
   ...
   alt="A 1.8-millimeter-wide field of H01 cortex showing the pial surface and thousands of cell bodies as small pale dots."
...
{% include figure.html
   ...
   alt="A 460-micrometer field of human cortex showing individual cell bodies and blood vessels."
...
{% include figure.html
   ...
   alt="A 115-micrometer field showing cell bodies embedded in dense neuropil."
...
{% include figure.html
   ...
   alt="A 29-micrometer field centered on a single neuronal cell body showing its nucleus, nucleolus, and surrounding neuropil."
...
{% include figure.html
   ...
   alt="A 3.6-micrometer field of human cortex at full 4-nanometer resolution showing membranes, mitochondria and synaptic vesicles."
...
H01 took 1.8 petabytes of raw data and, by the authors' count, 326 days of microscope
time.
...
- **Fixation speed governs everything.** Human surgical tissue cannot be perfusion-fixed
  the way an anesthetized mouse can; it was immersion-fixed immediately after excision.
  The authors report histological quality "equivalent to the rodent perfused cardiac
  samples used in the past," and conclude that **rapid immersion is a viable alternative
  to perfusion**. That is good news for every future human sample.
- **It is one specimen, from one person, with a neurological condition.** There is no
  second human cubic millimeter to compare it against.
...
- **Heavy-metal staining**: an **rOTO protocol** (reduced osmium / thiocarbohydrazide /
  osmium; the paper writes "ROTO"), following Tapia et al., *Nature Protocols* **7**, 193–206 (2012), followed by
  *en bloc* uranyl acetate. This is what creates contrast: everything dark in the
  figures above is heavy metal deposited on membrane.
- **Dehydration and embedding**: an ethanol series, then propylene oxide, then
  ...
  at tens of nanometers without tearing.
...
meaning a cut was skipped. Those sections are **duplicated in the digital stack** so
the z-geometry stays physically honest. That is why the released volume has **5,293
z-layers** (the 2021 preprint says 5,292) but only **5,019 physical sections** were cut.
If you have seen "~5,300 sections" quoted, that is where it comes from: it is the layer
count, not the section count.
...
<div class="callout-box callout-note">
  ...
  Every downstream algorithm has to cope with that. It is the main reason a thin
...
| **Total imaging time** | **326 days** (Shapson-Coe et al. 2024, PMC author manuscript PMC11718559; also the 2021 preprint) |
...
Parallel beams are one way to reach petascale throughput; MICrONS reached it instead
with five automated TEMs running side by side for about six months. Even at up to 190
million pixels per second, H01's imaging took 326 days, most of a year of continuous
microscope time.
...
{% include figure.html
   ...
   alt="A 7-micrometer field of human cortical neuropil at 8 nanometers per pixel, showing densely packed axons, dendrites and mitochondria."
...
- **Stitching**: assembling tiles within a section into one continuous image.
...
followed by a **fine-scale refinement based on optical flow between neighboring
...
were marked invalid** (2021 preprint) and handled separately, so they could not corrupt
...
<div class="callout-box callout-note">
  ...
  200 nm relative to its neighbor." Misalignment becomes split and merge errors, which
...
that grows one object at a time from a seed, repeatedly asking whether each neighboring
...
{% include figure.html
   ...
   alt="Raw electron microscopy of human cortical neuropil at 8 nanometers per pixel, grayscale."
...
{% include figure.html
   ...
   alt="The same electron microscopy field with the automated flood-filling network segmentation overlaid, each of 197 distinct objects in a different color."
   ...
   credit="Rendered from gs://h01-release/data/20210601/c2 over 4nm_raw. Colors are arbitrary, assigned per segment ID. CC BY 4.0." %}

### The two error modes are not symmetric

- A **split** breaks one true cell into several segments. Conservative: you under-report
  connectivity.
- A **merge** fuses two different cells. This is worse: it invents connections that do
  not exist and can contaminate a cell's entire reported connectivity.
...
Merge errors in the *base* segmentation were rare: **13 of 365,404 base segments, or
0.0036%**. Agglomeration is where merges get introduced, which is why two versions exist
at all. Automated cuts using a **six-class subcompartment model**
(axon, dendrite, soma, astrocyte, axon initial segment, cilium) removed millions of
suspect edges. Skeletonization used [kimimaro](https://github.com/seung-lab/kimimaro).
...
analyzed, **99.4% land on dendrites**, 0.39% on somata, and 0.20% on the axon initial
segment.
...
{% include figure.html
   ...
   alt="Electron microscopy at 4 nanometers per pixel with a red marker showing a real annotated excitatory synapse from the H01 dataset."
...
{% include figure.html
   ...
   alt="Electron microscopy at 4 nanometers per pixel with a blue marker showing a real annotated inhibitory synapse from the H01 dataset."
...
misclassification, they estimate **102.5 M excitatory (67.1%)** and **50.3 M inhibitory
(32.9%)**, a substantially different balance from the raw classifier output of 111.3 M
excitatory and 38.6 M inhibitory.
...
works through what this asymmetry does to an E/I ratio.

---

## Step 8 — Cell types, layers, and structure

With cells segmented and synapses attached, the volume can be organized anatomically.
...
| Glia | **32,315** |
| Neurons | **16,087** |
| of which spiny (8,803 pyramidal in shape) | 10,531 (65.5%) |
| of which non-spiny (classed as interneurons) | 4,688 (29.1%) |
| of which not classifiable (mostly somata cut by the volume edge) | 868 (5.4%) |
...
{% include figure.html
   ...
   alt="The H01 sample with its cortical layer segmentation overlaid in seven colors, showing layers 1 through 6 and white matter as curved bands."
   caption="H01's released layer segmentation over the EM. The sample spans the full cortical thickness, from layer 1 at the pial surface through layer 6 into white matter, which is what lets it support layer-specific claims at all. Boundaries curve because the cortex does."
...
- **104 of 16,087 neurons.** The paper says plainly that "it is infeasible for a single
  laboratory to proofread the entire dataset manually."
- **Dendritic spines were not proofread**, even on those 104 cells; there are hundreds
  ...
  (32.2% in c2, 33.7% in c3, per the PMC author manuscript; the two agglomerations
  barely differ here).

Wider proofreading continues through
**[CAVE](https://doi.org/10.1038/s41592-024-02426-z)**, a collaborative web-based
platform where anyone can apply to become a proofreader, and the current proofread
volume is browsable without applying. The release
...
**Deep-layer neurons come in two mirror-image groups.** In layers 5 and 6, **876
"triangular" neurons**, about a third of the spiny neurons there, have a large basal
dendrite emerging at a wide angle from the apical dendrite. Their orientations are **bimodal**:
...
means: **"What the function of this bimodal distribution of triangular cell basal dendrite
directions signifies remains to be determined."**

**Amid overwhelmingly weak connectivity, a few connections are very strong.**
The distribution of synapses per axonal input falls off steeply:
...
connections," human cortical neurons "appear to be innervated by a small subset of excitatory and
inhibitory inputs that purposefully establish more powerful connections." And a caveat they
...
forming extensive **whorls**. The authors write: "At present, we are unable to determine
whether these resulted from a pathological process or if they are simply rare." More are cataloged in the
...
- **This is not "the normal human cortex."** It is one cubic millimeter, from one person,
...
- **Normality can only be established by comparison**, and those comparisons do not exist
  yet: "Comparing samples obtained from individuals with different underlying disorders
  will allow for a better understanding of the phenomena seen here." Note also that fresh
...
- **A cubic millimeter truncates almost everything.** Axons and dendrites leave the box.
...
None of this makes H01 less valuable. It makes it a *first* human cubic millimeter rather
...
- **[Released data](https://h01-release.storage.googleapis.com/data.html)**: every layer
  with its exact `gs://` path, and the license.
- **[Code](https://h01-release.storage.googleapis.com/code.html)**: the software used at
  each stage above.
...
overlay reads `.../c2/` at a matching scale and colors by segment ID. The synapse markers
...
<div class="callout-box callout-note">
  <p><strong>License and attribution.</strong> All imagery on this page is derived from
  ...
  Note that the bioRxiv preprint carries a <em>different, more restrictive</em> license
  (CC BY-NC-ND 4.0) than the dataset. Do not conflate the two.</p>
</div>
