...
description: >
  ...
  now is license compliance and credit for proofreading labor; what is
...
- **License obligations** (§3). Major portals redistribute under
  different terms, and a reuser who ignores them is simply in breach.
- **Credit for proofreading labor** (§5). Tens of thousands of hours of human
...
urgent. Consent framings written for a cubic millimeter of discarded tissue
...
when you need to check what a license says or where a dataset came from; use
the module when you need to practice the judgment.
...
at the release landing page, and the license line covers the article, not the
...
- **Population size.** "A 45-year-old woman who had an anterior temporal
  resection for drug-resistant epilepsy, at a named center, in a narrow window
...
## 3. Licenses: what a reuser is actually obliged to do
...
FlyWire connectivity is a license problem; a commercial product built on MICrONS
connectivity is not.

Two more layers sit above the per-dataset license.
...
beyond what the dataset's own license grants.
...
fully open repositories where data may be downloaded and reanalyzed without
...
"the data is public" is not a license statement.
...
## 5. Credit for proofreading labor
...
labs, centralized teams at Princeton and Cambridge, and citizen scientists
...
number. **The absence
is itself a finding**: the largest single labor input to a connectome is
...
| **Collective acknowledgment in the author line** | Kim et al. (2014), *Nature* — the author list ends "and the EyeWirers", with the individual EyeWirers who reconstructed the cells listed in supplementary information | A verifiable named appearance, but in supplementary material | Not authorship; disappears from every automated record |
...
1. MICrONS: it is CC BY 4.0, which permits commercial use provided you
   attribute, mark modifications and link the license. FlyWire's data release
   is CC BY-NC 4.0, so a paid product is outside the license — even though
...
2. Because the volume never contained a name, or a face, or any direct
   ...
   in voxels.
3. Collective acknowledgment — the EyeWire model. The contributor loses
...
- **Legal advice, and jurisdiction-specific law.** GDPR special-category data,
  ...
  real and consequential; none is analyzed here, and nothing on this page is a
...
- **Unresolved license conflicts.** Hemibrain's project page (CC BY 4.0) and
  its v1.0 data deposit (CC BY-NC 4.0) disagree. The table applies the more
  restrictive terms until Janelia says which governs.
...
- [FlyWire whole-brain connectome]({{ '/content-library/case-studies/flywire-whole-brain/' | relative_url }})
  — the community whose labor §5 counts.
...
- [90-minute teaching package]({{ '/teaching/lectures/ethics-and-governance/' | relative_url }})
  — a timed plan, a license-and-credit worksheet and model responses.
