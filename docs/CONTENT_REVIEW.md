...
**1. The 25 module worksheets are still empty templates (~123 words each).**
They are generated by `scripts/generate_module_teaching_materials.rb` from module body
sections, so they contain headings and blank fill-in fields — "Claim: / Evidence: /
Limitation:" — with no task content, no data, and no answer key. Fixing them properly
means either enriching the module bodies the generator reads, or extending the generator
to pull worked-example and rubric content. Hand-editing them would be overwritten on the
next generator run.
...
**4. Two validation scripts fail under Ruby 3.3.** `scripts/validate_frontmatter.rb`
breaks on the Psych 4 `safe_load` keyword-argument change; `scripts/validate_figure_refs.rb`
breaks on a US-ASCII default external encoding. Both are environment issues rather than
content problems, but they mean two of the repo's four checks are not currently running.
...
1. **Worksheets.** Highest learner-facing impact per unit of effort. Extend the
   generator so worksheets carry the task, the data pointer, and the rubric.
...
4. **Fix the two validation scripts** so all four checks run in CI.
5. **Reconcile the `planned` status metadata** with the authored state.
