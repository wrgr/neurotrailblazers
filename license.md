...
| **Third-party figures and data** | Their own terms | Electron micrographs, dataset renderings, screenshots and figures reproduced from other projects; and every externally hosted dataset |

Copyright (c) 2025–2026 NeuroTrailblazers.

## What you may do with the content

Under CC BY 4.0 you may:

- **Teach from it.** Run a module, a whole track or the technical course, in a classroom,
  a lab meeting, a summer program or a workshop.
- **Change it.** Cut it down, translate it, re-order it, replace the datasets with ones
...
You do not need to ask permission. If you are running something with this material we
would like to hear about it — <a href="mailto:{{ site.email }}">{{ site.email }}</a> —
but that is a request, not a condition.

## How to attribute

For a page or a set of pages:

> Adapted from *NeuroTrailblazers*, <{{ site.url }}/>, licensed CC BY 4.0.

On a slide, a footer line is enough: **NeuroTrailblazers · CC BY 4.0**. If you modified
the material, say so — "adapted from" does that work. For a formal citation, see
[Cite this site]({{ '/about/#cite-this-site' | relative_url }}).

## Figures are the exception — check every one

**This is the part that catches people.** A large share of the images on this site are
electron micrographs, segmentation renderings and figures from other projects — H01,
MICrONS, FlyWire, the Allen Institute, published papers. They are reproduced here under
their own terms, which are sometimes narrower than CC BY 4.0 and which this project
cannot widen.
...
Ask: <a href="mailto:{{ site.email }}">{{ site.email }}</a>. If you think a figure on this
site is mis-credited or should not be here,
[open an issue](https://github.com/{{ site.github_username }}/neurotrailblazers/issues)
or email us and we will take it down while we check.
