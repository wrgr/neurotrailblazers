...
async function main() {
  ...
  // The EZProxy session token is a credential tied to a personal JHU account. It is
  // read from jhu_cookie.txt (gitignored) or JHU_EZPROXY_TOKEN, never committed. An
  // earlier revision of this file carried one as a hardcoded fallback; that token is
  // to be treated as compromised. Fail loudly rather than falling back to anything.
  let token = (process.env.JHU_EZPROXY_TOKEN || '').trim();
  if (!token && fs.existsSync(COOKIE_PATH)) {
    token = fs.readFileSync(COOKIE_PATH, 'utf8').trim();
  }
  if (!token) {
    console.error('No EZProxy session token.');
    console.error(`Write one to ${COOKIE_PATH}, or set JHU_EZPROXY_TOKEN.`);
    console.error('Both are gitignored. Do not paste a token into this file.');
    process.exit(1);
  }
