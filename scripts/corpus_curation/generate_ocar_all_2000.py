#!/usr/bin/env python3
"""Generates complete OCAR cards, 3-tier pedagogical summaries, and discussion prompts for all 2,000 papers."""
import json
import os
import re
...
if PDF_DIR.exists():
    for f in PDF_DIR.glob("*.pdf"):
        AVAILABLE_PDFS[f.name.lower()] = f

def extract_pdf_text_if_available(doi: str) -> Optional[str]:
    if not AVAILABLE_PDFS: return None
    clean_d = sanitize_doi(doi).replace("/", "_")
    target_name1 = f"doi_{clean_d}.pdf".lower()
    target_name2 = f"{clean_d}.pdf".lower()
    target_pdf = AVAILABLE_PDFS.get(target_name1) or AVAILABLE_PDFS.get(target_name2)
    if target_pdf:
        try:
            import subprocess
            res = subprocess.run(["pdftotext", str(target_pdf), "-"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=2)
...
def main():
    ...
    abstracts_dict = json.loads(abs_path.read_text()) if abs_path.exists() else {}

    print(f"Generating rich OCAR cards and 3-tier summaries for ALL {len(papers)} curated papers...")

    # Load expert seed papers if available
    expert_cards = {}
    ...
    print(f"Loaded {len(expert_cards)} hand-curated expert seed cards.")

    processed_papers = []
    
    # Sort papers by tier then citation centrality
    tier_order = {"500": 1, "1000": 2, "2000": 3}
    ...
    for doi in sorted_dois:
        ...
        # Check if full abstract exists
        raw_abs = abstracts_dict.get(clean_doi, "")
        if isinstance(raw_abs, str):
            full_abs = raw_abs
        elif isinstance(raw_abs, dict):
            full_abs = raw_abs.get("abstract", "")
        else:
            full_abs = ""
        ...
        # Check expert seed
        if clean_doi in expert_cards and expert_cards[clean_doi].get("ocar") and expert_cards[clean_doi]["ocar"].get("opportunity"):
            ...
            # Check local PDF
            pdf_txt = extract_pdf_text_if_available(clean_doi)
            if pdf_txt:
                synth = synthesize_ocar_and_summaries(p, pdf_txt, "generated_from_fulltext_pdf")
            else:
    ...
    # Save to _data/corpus_500.json, _data/corpus_1000.json, _data/corpus_2000.json
    ...
    (PROJECT_ROOT / "_data/corpus_1000.json").write_text(json.dumps({
        "metadata": {
            ...
            "tier": 1000,
            "count": len(c1000),
            "description": "1000 Key Papers in Connectomics — Comprehensive landmark literature corpus with complete OCAR cards and unabridged abstracts."
        },
        "papers": c1000
    }, indent=2))
    ...
    (PROJECT_ROOT / "_data/corpus_2000.json").write_text(json.dumps({
        "metadata": {
            ...
            "tier": 2000,
            "count": len(c2000),
            "description": "2000 Key Papers in Connectomics — Full research network with complete OCAR cards, 3-tier summaries, and 5,460+ directed citation edges."
        },
        "papers": c2000
    }, indent=2))
