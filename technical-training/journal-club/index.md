...
<section class="jc-hero">
  ...
  <p>{{ site.data.journal_papers.papers.size }} connectomics papers, chosen by their place in the field's citation graph. Each card gives the paper's citation and published abstract, plus OCAR notes (Opportunity, Challenge, Action, Resolution, Future Work), three-level summaries and discussion prompts. The OCAR notes, summaries and prompts are written per research domain and shared across it; the abstract is what the paper itself reports.</p>
...
<section class="section section-compact">
  <p>Browse the corpus across {{ jc_dims.size }} research domains and 3 nested tiers. Use this page to survey what exists, filter by domain/organism/era, or follow the citation graph between papers. To see which papers cite which, open the citation graph:</p>
...
<section class="section section-compact">
  ...
  <p class="jc-shown-note">Showing <strong id="jc-shown-tier-label">500 Key Papers</strong> in Connectomics (stratified across {{ jc_dims.size }} domains) with abstracts, domain-level OCAR notes and three-level summaries. Use the tier selector above to expand to 1,000 or 2,000 papers, or explore the <a href="{{ '/technical-training/journal-club/graph/' | relative_url }}">interactive citation graph</a>.</p>
  ...
  function generatePromptContent() {
    if (singlePaperTarget) {
      ...
      var prompt = 'You are a principal investigator and expert seminar leader in connectomics.\n' +
        'Please provide a scientific evaluation and discussion guide for the following paper. Note: the OCAR notes below are shared by every paper in its research domain and are not a summary of this paper; base your answer on the paper itself, and say so if you cannot read it rather than guessing.\n\n' +
        '### Paper Metadata:\n' +
        '- **Title**: "' + p.title + '"\n' +
        '- **Authors**: ' + p.authors + '\n' +
        ...
        '- **Resolution (typical for the domain)**: ' + p.resolution + '\n' +
    ...
    var scopeDesc = totalCount <= 35
      ...
      : 'based on the top ' + maxIncluded + ' publications by citation centrality (from ' + totalCount + ' matching papers in the active filter)';
